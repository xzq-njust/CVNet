...
def plot_sample_eval_contours(image, mask, alpha, beta, data, final_x, final_y,
                sequence_id, save_folder, caption=None):
    plt.imshow(image)
    plt.plot(final_x, final_y, '--', lw=3, color=[1, 1, 0])
    plt.gca().xaxis.set_major_locator(plt.NullLocator())
    plt.gca().yaxis.set_major_locator(plt.NullLocator())
    plt.subplots_adjust(top = 1, bottom = 0, right = 1, left = 0, hspace = 0, wspace = 0)
    plt.margins(0,0)
    plt.savefig(os.path.join(save_folder, "{}.png".format(sequence_id)))
    matplotlib.image.imsave(os.path.join(save_folder, "alpha_{}.png".format(sequence_id)), alpha)
    matplotlib.image.imsave(os.path.join(save_folder, "beta_{}.png".format(sequence_id)), beta)
    plt.close('all')
