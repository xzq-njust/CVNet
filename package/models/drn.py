...
class DRN(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.arch == 'C':
            ...
            x = self.layer0(x) # [16, H, W]

        x = self.layer1(x) # [16, H, W]
        y.append(x)
        x = self.layer2(x) # [32, H/2, W/2]
        y.append(x)

        x = self.layer3(x) # [64, H/4, W/4]
        y.append(x)

        x = self.layer4(x) # [128, H/8, W/8]
        # y.append(x)
...
#     def __init__(self, block, layers, num_classes=1000,
#                  channels=(16, 32, 64, 128, 256, 512, 512, 512),
#                  out_map=False, out_middle=True, pool_size=28, arch='D'):
#         super(DRN, self).__init__()
#         self.inplanes = channels[0]
#         self.out_map = out_map
#         self.out_dim = channels[-1]
...
def drn_d_22(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        # model.load_state_dict(torch.load('/test/CVNet/drn_d_22-4bd2f8ea.pth'))
    return model
...
def drn_d_38(pretrained=False, **kwargs):
    ...
    if pretrained:
        # model.load_state_dict(torch.load('/test/CVNet/drn_d_38-eebb45f0.pth'))
