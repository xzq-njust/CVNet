...
from package.models import drn
from package.models.drn import BasicBlock
...
class DRNContours(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.upconv2(x)
        # x = torch.cat((x, y[-2]), dim=1)
        x = self.upconv3(x)
        # x = torch.cat((x, y[-3]), dim=1)
        ...
        # return beta1, data1, beta2, data2
...
#         self.upconv1 = UpConvBlock(model.out_dim, model.out_dim // 2)
#         self.upconv2 = UpConvBlock(model.out_dim // 2 + 64, model.out_dim // 4)
#         self.upconv3 = UpConvBlock(model.out_dim // 4 + 32, model.out_dim // 8)
#         self.predict_1 = nn.Conv2d(model.out_dim // 8 + 16, classes, 
#                                    kernel_size=3, stride=1, padding=1, bias=True)

#         # Combine predictions with output of upconv 3 to further refine
#         self.conv4 = self._create_convblock(model.out_dim // 8 + classes + 16, model.out_dim // 16, 1)
#         self.conv5 = self._create_convblock(model.out_dim // 16, model.out_dim // 32, 2)
#         self.conv6 = self._create_convblock(model.out_dim // 32, model.out_dim // 32, 1)
#         self.predict_2 = nn.Conv2d(model.out_dim // 32, classes, 
#                                    kernel_size=3, stride=1, padding=1, bias=True)
