...
from package.utils.data_utils import draw_circle, draw_poly_mask


class BaseDataset(torch.utils.data.Dataset):
    def __init__(self, mode, image_ext, init_contour_radius,
    ...
    def interpolate_gt_snake(self, pgtnp_px2):
        newpnum = self.discretize_points
    ...
    def resort_gt_snake(self, snake):
        # snake, [L,2]
        center_snake = np.mean(snake, axis=0)
        left_points = snake[:,0] <= center_snake[0]
        top_points = snake[:,1] <= center_snake[1]
        u = snake.copy()
        dif_u_c = u - center_snake
        dif_u_c[left_points | top_points] = 256
        index = dif_u_c[:,1].argmin(axis=0)
        if dif_u_c[index+1][1] != 256:
            flag = 1
            snake = np.concatenate((snake[index:,:], snake[0:index,:]), axis=0)
        else:
            flag = -1
            snake = np.concatenate((np.flipud(snake[0:index+1,:]), np.flipud(snake[index+1:,:])), axis=0)
        return snake
    ...
    def random_scale(self, image, masks_to_scale, polygon, padding_mode=['reflect','constant'],
        ...
        if min_r == 0:
            return image, masks_to_scale, polygon
        else:
    ...
    def initialize_contour(self, origin, ratio_radius=1.0):
        # circle
        assert len(origin) == 2
        angles = np.linspace(0, 2 * np.pi, self.discretize_points, endpoint=False)
        init_u = origin[0] + ratio_radius * self.init_contour_radius * np.cos(angles)
        init_v = origin[1] + ratio_radius * self.init_contour_radius * np.sin(angles)
        init_u = init_u.reshape([self.discretize_points, 1])
        init_v = init_v.reshape([self.discretize_points, 1])
        init_contour = np.array([init_u[:, 0], init_v[:, 0]]).T
        return init_contour

    def get_faces(self, contour):
        half_dim =  self.final_size / 2
        contour = (contour - half_dim) / half_dim
