...
class BingDataset(BaseDataset):
    ...
    def __init__(self, mode, final_size=256, discretize_points=60,
        init_contour_radius=12, normalize=True):
        ...
        self.color_jitter = transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.01)
        ...
        for path in self.image_paths:
            ...
            num_vertices = 4
            polygon = np.array(df.iloc[sequence_id][1:num_vertices * 2 + 1].tolist()).reshape(num_vertices, 2)
    ...
    def get_masks(self, idx):
        ...
        mask_ending_all = image_ending.replace("building_", "building_mask_all_")
    ...
    def __getitem__(self, idx):
        ...
        if 'train' in self.mode:
            # d_x = np.random.randint(-2, 3)
            # d_y = np.random.randint(-2, 3)
            # polygon[:,0] = polygon[:,0] + d_x
            # polygon[:,1] = polygon[:,1] + d_y
            # polygon = np.maximum(polygon, 0)
            # polygon = np.minimum(polygon, 255)
